...
def _make_server(*, name: str = "TestServer", server_type: str = "plex") -> MagicMock:
    """Create a minimal MediaServer-like mock."""
    server = MagicMock(spec=MediaServer)
    server.id = uuid4()
    server.name = name
    server.server_type = server_type
    server.url = f"http://{name.lower()}.local:32400"
...
def _make_redemption_service() -> tuple[RedemptionService, AsyncMock, AsyncMock]:
    ...
    user_service.user_repository.get_by_username_and_server = AsyncMock(
        return_value=None
    )
...
class TestRedemptionRetry:
    ...
    async def test_create_user_retries_on_external_service_error(self) -> None:
        ...
        dns_error = ExternalServiceError(
            server.url, "DNS resolution failed", original=OSError("Name resolution")
        )
    ...
    async def test_create_user_retries_exhausted_triggers_rollback(self) -> None:
        ...
        assert mock_registry.create_client_for_server.call_count == 6
