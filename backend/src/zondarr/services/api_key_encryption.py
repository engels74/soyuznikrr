"""API key encryption using Fernet with HKDF-derived key.
...
def _derive_key(secret_key: str) -> bytes:
    ...
    hkdf = HKDF(
        ...
        length=32,
        salt=b"zondarr-api-key",
        info=b"media-server-api-key",
    )
...
def encrypt_api_key(plaintext: str, /, *, secret_key: str) -> str:
    """Encrypt a media server API key for database storage.
...
def decrypt_api_key(ciphertext: str, /, *, secret_key: str) -> str:
    """Decrypt a media server API key from database storage.
...
__all__ = ["InvalidToken", "decrypt_api_key", "encrypt_api_key"]
